...
namespace Api.Test.Transaction.ChangeAmount
{
    public class ChangeAmountTest : IClassFixture<CustomWebApplicationFactory>
    {
        private readonly HttpClient _httpClient;
        private readonly CustomWebApplicationFactory _factory;

        public ChangeAmountTest(CustomWebApplicationFactory factory)
        {
            _factory = factory;
        ...
        public async Task Success_Debit()
        {
            ...
            var response = await _httpClient.PatchAsJsonAsync($"/transactions/{card.Id}/{transaction.Id}/amount", request);
            ...
            var savedTransaction = await dbContext.Transactions.FirstAsync(t => t.Id == transaction.Id);

            savedTransaction.Amount.Should().Be(150);

            var updatedCard = await dbContext.Cards.FirstAsync(c => c.Id == card.Id);

            updatedCard.Debit.Should().Be(850);
        }
        ...
        public async Task Success_Credit()
        {
            ...
            updatedCard.CreditBalance.Should().Be(650);
        }
        ...
        public async Task Error_Transaction_Not_Found(string culture)
        {
            ...
            var response = await _httpClient.PatchAsJsonAsync($"/transactions/{card.Id}/99999/amount", request);
        ...
        public async Task Error_Transaction_Of_Other_Card(string culture)
        {
            ...
            var response = await _httpClient.PatchAsJsonAsync($"/transactions/{otherCard.Id}/{transaction.Id}/amount", request);
        ...
        public async Task Error_Amount_Invalid(string culture)
        {
            ...
            await AssertSingleError(response, "CHANGE_AMOUNT", culture);
        }
        ...
        public async Task Error_Credit_Limit_Exceeded(string culture)
        {
            ...
            var transaction = await _factory.SeedTransactionAsync(card, CardManager.Domain.Entities.PaymentMethod.Credit, 100, "Compra");
        ...
        public async Task Error_No_Token()
        {
            ...
            var response = await _httpClient.PatchAsJsonAsync("/transactions/1/1/amount", request);
